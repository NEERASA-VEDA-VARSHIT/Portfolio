  ...
  return (
    <div className="fixed left-23 top-4/6 transform -translate-y-1/2 z-20 opacity-60">
      <div className="flex flex-col items-center space-y-6">
        ...
        <VerticalText text="FOLLOW ME" />
      </div>
    </div>
  );
};

const SocialLink = ({ href, icon, hoverColor, platform }) => {
  const icons = {
  ...
  return (
    <a
      ...
      title={`Follow me on ${platform}`}
    >
      <svg
        className={`w-5 h-5 fill-gray-600 group-hover:fill-${hoverColor} transition-colors duration-300`}
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
        <path d={icons[icon]} />
